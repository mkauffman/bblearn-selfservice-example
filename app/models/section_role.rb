...
class SectionRole < ActiveRecord::Base
  ...
  def destroy
    role = self.attributes.to_options
    section_role_web_service.delete_course_membership(role)
  end

  def self.create(crsmain_pk1, users_pk1, role_id)
    section_role_web_service.save_course_membership   :crsmain_pk1 => crsmain_pk1,
                                                      :users_pk1 => users_pk1,
                                                      :role_id => role_id
  end
