...
module Savon
  ...
  class WSSE
    ...
    def credentials(password, digest = false)
      self.username = "session"
      self.password = password || "nosession"
      self.digest = digest
    end
  ...
  private
    ...
    #Returns modified hour time for Timestamp and Security details
    def server_hour
        hour = Time.now.strftime("%H")
        hour = hour.to_i + 10
    end

    #Returns modified minute time for Timestamp and Security details
    def expired_min
    ...
    def wsse_timestamp
      wsse_security "Timestamp",
        "wsu:Created" => created_at || Time.now.strftime("%Y-%m-%dT#{server_hour}:%M:%SZ"),
 #       "wsu:Created" => ((created_at || Time.now) + 10800).xs_datetime,
        "wsu:Expires" => expires_at || Time.now.strftime("%Y-%m-%dT#{server_hour}:#{expired_min}:%SZ")
 #       "wsu:Expires" => (expires_at || (created_at || Time.now) + 10860).xs_datetime
    end
    ...
    def wsse_security(tag, hash)
      {
        ...
        "wsu:Timestamp" => {
            "wsu:Created" => created_at || Time.now.strftime("%Y-%m-%dT#{server_hour}:%M:%SZ"),
	      #  "wsu:Created" => (created_at || (Time.now + (60 * 60 * 5))).to_s,
            "wsu:Expires" => expires_at || Time.now.strftime("%Y-%m-%dT#{server_hour}:#{expired_min}:%SZ")
          #  "wsu:Expires" => (expires_at || (created_at || Time.now) + (60 + (60 * 60 * 5))).to_s 
        	},
